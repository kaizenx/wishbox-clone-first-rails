class ContactUsController < ApplicationController
  ...
  def contact
    if request.post?
      first_name = params[:ContactUsFirstName]
      last_name = params[:ContactUsLastName]
      email = params[:ContactUsEmail]
      comment = params[:ContactUsComments]
      contact_us = ContactU.where(:FirstName => first_name, :LastName => last_name, :Email => email, :Comment => comment ).take
      if contact_us == nil
        contact_us = ContactU.new(:FirstName => first_name, :LastName => last_name, :Email => email, :Comment => comment )
        contact_us.save
        @notice = "Message successfully created, we will process it shortly."
        # flash[:notice] = "Message successfully created, we will process it shortly."
      end 
    end
  end
  # GET /contact_us
  ...
  private
    ...
    def contact_u_params
      params.require(:contact_u).permit(:FirstName, :LastName, :Email, :Comment)
    end
end
